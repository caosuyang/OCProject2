//
//  XBJinRRChangJiePayViewController.m
//  Jinrirong
//
...
@interface XBJinRRChangJiePayViewController ()
...
@implementation XBJinRRChangJiePayViewController
...
-(void)payBtnClick{
    ...
    if (isMessage) {
        ...
        [XBJinRRNetworkApiManager Center_Payment_NowPayConfirmWithOrderNo:self.OrderNo Smscode:_cHXBankModel.Smscode Block:^(id data) {
            [weakSelf.view hide];
...
//        }];
        ...
        [XBJinRRNetworkApiManager Center_Payment_NowPayWithOrderNo:self.OrderNo BankNo:_cHXBankModel.BankNo CertNo:_cHXBankModel.CertNo CertName:_cHXBankModel.CertName CertPhone:_cHXBankModel.CertPhone Block:^(id data) {
            [weakSelf.view hide];
            if ([data[@"result"]intValue]==1) {
                self->isMessage = YES;
                [Dialog toastCenter:[NSString stringWithFormat:@"%@",data[@"message"]]];
