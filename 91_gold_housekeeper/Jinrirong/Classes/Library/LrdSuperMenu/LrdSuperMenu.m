...
#define kTextSelectColor MainColor
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        _selectedTextColor = [UIColor colorWithRed:74 green:87 blue:232 alpha:1];
...
- (void)setDataSource:(id<LrdSuperMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numberOfColumn; i++) {
        ...
        //indicatorLayer
        CGPoint indicatorPosition = CGPointMake((i + 1) * numberOfLine - 20, self.height / 2);
...
- (CATextLayer *)createTitleLayerWithString:(NSString *)string position:(CGPoint)position color:(UIColor *)color {
    ...
    layer.bounds = CGRectMake(0, 0,(self.frame.size.width / _numberOfColumn) - 28 , size.height);
    layer.string = string;
...
- (void)animateTableView:(UITableView *)tableview show:(BOOL)show complete:(complete)complete {
    ...
    if (show) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                self->_leftTableView.frame = CGRectMake(self.origin.x, self.origin.y + self.height, SCREEN_WIDTH / 2, tableViewHeight);
                self->_rightTableView.frame = CGRectMake(self.origin.x + SCREEN_WIDTH / 2, self.origin.y +self.height, SCREEN_WIDTH / 2, tableViewHeight);
            }else {
                self->_leftTableView.frame = CGRectMake(self.origin.x, self.origin.y + self.height, SCREEN_WIDTH, tableViewHeight);
            }
        }];
    }else {
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                self->_leftTableView.frame = CGRectMake(self.origin.x, self.origin.y + self.height, SCREEN_WIDTH / 2, 0);
                self->_rightTableView.frame = CGRectMake(self.origin.x + SCREEN_WIDTH / 2, self.origin.y +self.height, SCREEN_WIDTH / 2, 0);
            }else {
                self->_leftTableView.frame = CGRectMake(self.origin.x, self.origin.y + self.height, SCREEN_WIDTH, 0);
            }
        } completion:^(BOOL finished) {
            if (self->_rightTableView) {
...
- (void)animateTitle:(CATextLayer *)title show:(BOOL)show complete:(complete)complete {
    ...
    CGFloat sizeWidth = (size.width < (self.frame.size.width / _numberOfColumn) - 25) ? size.width : self.frame.size.width / _numberOfColumn - 25;
    title.bounds = CGRectMake(0, 0, (self.frame.size.width / _numberOfColumn) - 28, size.height);
    if (!show) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        ...
        cell.textLabel.frame = CGRectMake(frame.origin.x, 5, frame.size.width, frame.size.height-10);
        
        //        cell.textLabel.layer.borderColor = MainColor.CGColor;
...
- (void)menuTapped:(UITapGestureRecognizer *)gesture {
    ...
    if (touchIndex == _currentSelectedColumn && _isShow) {
        ...
        [self animateIndicator:_indicators[touchIndex] background:_backGroundView tableView:_leftTableView title:_titles[touchIndex] reverse:NO complecte:^{
            self->_currentSelectedColumn = touchIndex;
            self->_isShow = NO;
        }];
    }else {
        //弹出menu
    ...
    if (_delegate &&[_delegate respondsToSelector:@selector(menu:didSelectIndex:IsShow:)]) {
        [_delegate menu:self didSelectIndex:touchIndex IsShow:_isShow];
    }
...
- (void)backTapped:(UITapGestureRecognizer *)gesture {
    ...
    if (_delegate &&[_delegate respondsToSelector:@selector(backClickBlock)]) {
        [_delegate backClickBlock];
    }
